...
export default {
  install: (Vue, options) => {
    ...
    const bucketConfigurations = process.env.NODE_ENV !== 'production' ? options.cdnConfig : null

    Vue.getBucketConfigByKey = (bucketId) => {
      const result = bucketConfigurations.find(bucket => bucket.bucketId === bucketId)
      return result
    }
    ...
    Vue.getServerList = function (purposeId, bucketId) {
      const result = cdnServerList.find(purpose => (purpose.purposeId === purposeId && purpose.bucketId === bucketId))
      return result
    }
